...
服务额外接入 Elasticsearch：对象以 etcd 为主存储，创建/更新时同步写入 ES，读取单对象时会尝试从 ES 取回增强数据并覆盖返回对象。ES 主要作为搜索与扩展数据承载的基础。
...
apiserver 使用 `k8s.io/apiserver/pkg/registry/generic/registry.Store` 作为资源主存储，默认前缀为：
