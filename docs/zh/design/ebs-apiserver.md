...
- `RpmRepo` 创建时还要求 `metadata.labels[ebs.io/target-os]` 与 `metadata.labels[ebs.io/target-arch]` 存在且非空（由 Build Controller 写入，取值等于同名 Build 的 `spec.buildTarget.os` / `arch`），缺失或为空返回 422；该校验只在创建路径生效，不影响普通更新与 `/status` 更新。
...
- Job 创建时若带有 `ebs.io/build-name`（由 BuildInfo Controller 创建的构建 Job），还必须带有 `ebs.io/spec-name` 和 `ebs.io/package-name` label；apiserver 校验标签存在且值符合 Kubernetes label 规则，缺失或非法返回 `422 Invalid`。包名标签由 BuildInfo Controller 按[标签约定](labels.md#7-job-构建归属标签)从 `BuildInfo.spec.specDepends[specName].repoName` 计算。普通更新不得修改已有归属标签，`/status` 更新保留原 metadata。没有 `ebs.io/build-name` 的通用 Job 不强制要求这些字段；存量构建 Job 即使缺少包名标签，普通更新和状态更新仍允许，且不会自动回填。
