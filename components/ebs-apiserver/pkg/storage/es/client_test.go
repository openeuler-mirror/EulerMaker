...
func TestEnsureIndicesOnlyCreatesESPrimaryResources(t *testing.T) {
	...
	for _, index := range []string{"ebs-projects-v1", "ebs-snapshots-v1", "ebs-builds-v1", "ebs-buildinfos-v1", "ebs-rpmrepos-v1", "ebs-configs-v1", "ebs-build-target-claims-v1"} {
		if !created[index] {
	...
	if strings.Contains(mappings["ebs-rpmrepos-v1"], `"repository"`) ||
		...
		t.Fatalf("rpmrepo index should map only release status: %s", mappings["ebs-rpmrepos-v1"])
	}
